...
whichMeterType = 1;
PortString = 'USB0';    % djs works for theeeg booth
...
thePrompt = sprintf('Enter calibration filename (include folder) [%s]: ',defaultFileName);
newFileName = input(thePrompt,'s');
newFileNameDir = [];
if isempty(newFileName)
    newFileName = defaultFileName;
end
[newFileNameDir, n, e]=fileparts(newFileName);
newFileName = [n, e];
...
% DJS HARD-CODE
if whichMeterType~=1
    error('Hard-coded to use only PR-650.');
end
...
% Calibrate monitor
USERPROMPT = 1;
cal = djsCalibrateMonDrvr(cal, USERPROMPT, whichMeterType, blankOtherScreen);
...
% Save the structure
fprintf(1, '\nSaving to %s.mat\n', newFileName);
SaveCalFile(cal, newFileName, newFileNameDir);
