function cal = djsCalibrateMonDrvr(cal, USERPROMPT, whichMeterType, blankOtherScreen)
...
fprintf('djsCalibrateMonSpd: Using syncMode=off\n');
syncMode='off';
...
for a = 1:cal.describe.nAverage
    for i = 1:cal.nDevices
        ...
        % Measure ambient
        darkAmbient1 = djsMeasMonSpd(window, [0 0 0]', cal.describe.S, syncMode, whichMeterType, theClut);
        ...
        fprintf(1,'djsMeasMonSpd run %g, device %g\n',a,i);
        [tempMon, cal.describe.S] = djsMeasMonSpd(window, mGammaInput(:,sortIndex), ...
            cal.describe.S, syncMode, whichMeterType, theClut);
        tempMon(:, sortIndex) = tempMon;

        % Take another ambient reading and average
        darkAmbient2 = djsMeasMonSpd(window, [0 0 0]', cal.describe.S, syncMode, whichMeterType, theClut);
        ...
        % turn off and on
        if i<cal.nDevices
            fprintf('Close and reopen PR650...\n');
            CMClose(1);
            fprintf('Cycle power on PR650, hit ENTER when ready....\n');
            KbStrokeWait(-1);
            fprintf('Re-connect to PR650...\n');
            CMCheckInit(1, 'USB0');
            fprintf('PR650 ready.\n');
        end
    end
